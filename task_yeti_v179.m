...
kex_act = 0;
nval = 7;
gblist = [0.1, 0.2, 0.5, 1., 2., 5., 10.];
xtask = mod(itask, nval);
gb = 3.1*gblist(xtask + nval*(xtask==0));
disp(['gamma_bead = ' num2str(gb)])
ga_rat = 2.;
...
for t = loadj:tprint:loadj + totaltime - 1
    ...
    if real_t_min >=0
        [binding_rate_myo2, binding_rate_myp2, binding_rate_for, vpol, rsev]=...
            modify_binding_rates_v2( real_t_min, binding_rate_myo2_init, binding_rate_myp2_init, binding_rate_for_init, kofffor, r, r_ring, rhof, koffmyo);
        % septation rate (absolute value; defined as dr/dt; .07 micron / minute in Pelham and Chang)
        vs_abs = 0;
    end
    rk1_v45
