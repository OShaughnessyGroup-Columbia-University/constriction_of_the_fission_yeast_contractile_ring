...
% v14 to 17 skipped
% change from v18: always run the update loop if nstep == 1
% change from v19: use update_v10 and give dt as argument. 
% change from v20: use bm_nbr and bp_nbr throughout.
% change from v21: use get_force_v6
% change from v22: use update_v11
% change from v23: use get_force_v7
% change from v24: use get_force_v8
% change from v25: run the update block every time step
% change from v26: use update_v12
% change from v27: use update_v13
% change from v28: use update_v14
% change from v29: use update_v15
% change from v30: use get_force_v9
% change from v31: use get_force_v10 and update_v16
% change from v32: track entry times for nodes, beads, myps
% change from v44: use get_a_v2 and get_m_v2 to change artificial drag
...
% %             poly_time = false;
% %         end
        [rbead,rmyo,rmyp,xmat,fmmat,ipt,ifor,bancf,bancm,dbead_first,bpmat_trial,t_rbead,t_rmyo,t_rmyp] = update_v22(rbead,rmyo,rmyp,bpmat,xmat,rhoain,rxbind,rex_act,rexc_n,fmmat,bancf,bancm,ifor,ipt,kofffor,...
            koffmyo,koffmyp,dt,d_for,d_myo,dbead,dbead_first,rsev,koffx,rcapmyp,l_break_sq,r_ring,wr,...
            binding_rate_myo2,binding_rate_myp2,binding_rate_for,vpol,geometry,real_t_min, t_rbead,t_rmyo,t_rmyp);
        bmmat = bm_nbr (rbead, rmyo, ifor, ipt, rcapmyo_long, rcapmyo_short, fmmat);
        bpmat = bp_nbr (rbead, rmyp, ifor, ipt, rcapmyp);
        rdiff = get_rdiff(rbead,ifor,ipt); 
        nsat_p = nsat;
        [force,force_vec, gbeadfv, gppf, bmmatpt, bpmatpt] = get_force_v10(rbead,rmyo,rmyp,rdiff,ifor,ipt,bancm,bmmat,bpmat,nhead,fhead,fheadmyp,fone,nsat,rcapmyo_u, rcapmyo_long, rcapmyo_short,rcapmyp,kcap,kcap_p,r_flat_myo,r_flat_myp,dbead,vmyo0,kod2,r_ring,kwall,kexc,kexc_po,rexc,rexc_n,rexc_po,kex_act,rex_act,xmat,rx0,kx,d_cdc15,d_myo,geometry);
        amat = get_a_v2(rbead, rmyo, rmyp, bmmat, bpmat, gppf, rdiff, dt, kcap, kcap_p, ifor, ipt, bmmatpt, bpmatpt, ga_rat);
        if geometry == 'cyl'
...
%         w = get_w(ga,ifor,ipt,rbead,rmyo,bancf,bancm,bmmat,rdiff,gbeadfv,gppf,gb,gf,gm,gmsol,bmmatpt);
        m = get_m_v2 (dt,kcap,kcap_p,ifor,ipt,rbead,rmyo,rmyp,bancf,bancm,bmmat,bpmat,rdiff,gbeadfv,gppf,gb,gf,gm,gmsol,bmmatpt,bpmatpt, ga_rat);
        m = m - amat;
        tau = tau_o_dt * dt;
        nbead = size(rbead,2);
