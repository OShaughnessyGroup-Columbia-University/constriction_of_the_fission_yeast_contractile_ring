...
%                 r_new_for = rmyo(:,myo_ind);    
                ...
                for inuc = 1:100
                    ...
                    %if temp(myo_ind)>0              % cannot bind to myosin already occupied by 4 formin dimers
                    %    continue
                    %end
                    temp = sum(fmmat,1); % total number of formins on each myosin
                    mzero = find(temp == 0); % myosins w/ no formin
                    if(~isempty(mzero))
                        idx = randi(size(mzero));
                        myo_ind = mzero(idx);
                    else
                        disp('Skipping this formin--nowhere to go!')
                        break
                    end
