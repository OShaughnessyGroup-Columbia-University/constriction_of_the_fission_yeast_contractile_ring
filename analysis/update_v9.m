...
function [rbead,rmyo,rmyp,xmat,fmmat,ipt,ifor,bancf,bancm,dbead_first,bpmat_trial] = update_v9(rbead,rmyo,rmyp,bpmat,xmat,rhoain,rxbind,rex_act,fmmat,bancf,bancm,ifor,ipt,kofffor,...
...
%                 r_new_for = rmyo(:,myo_ind);    
                ...
                for inuc = 1:100
                    ...
                    temp = rbead - vrand;
                    temp = sum(temp.*temp);
                    % if vrand is not within rex_act of any bead, end
                    if all(temp > rex_act^2)
                        break
                    end
                    if inuc == 100
