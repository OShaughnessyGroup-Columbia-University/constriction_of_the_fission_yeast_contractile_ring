...
yyaxis left
% ylim([0, 0.2])
errorbar(gamma_anch, vnode_mean, vnode_std/sqrt(5), 'o', 'LineWidth', 2, ...
    'MarkerFaceColor', 'auto', 'MarkerSize', 10, 'CapSize', 12)
plot(gm_theory, vnode_theory, 'LineWidth', 3)
plot(xlim, [.028, .028], '--', 'LineWidth', 2)
% ylabel('v_{node} (\mum/s)')
ylabel('v_{node} (mm/s)')
yyaxis right
errorbar(gamma_anch, tens_ave, tens_std/sqrt(5), 'o', 'LineWidth', 2, ...
    'MarkerFaceColor', 'auto', 'MarkerSize', 10, 'CapSize', 12)
plot(gm_theory, t_theory, 'LineWidth', 3)
plot(xlim, [640 640], '--', 'LineWidth', 2)
plot([2e3,2e3], ylim, 'k--', 'LineWidth', 2)
xlim([0, 3500])
xlabel('g_{anch} (pN s/mm)')
