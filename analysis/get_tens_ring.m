...
% by fitth. 
    ...
    % tangent unit vectors for fitth
    tg = [-sin(fitth);cos(fitth);zeros(size(fitth))];
    % signs of tension -- positive is tensile
    sgntens = -sign(sum(tg .* vtens));
    % tension
    tens = abs(sum(tg .* vtens));
%     tens = -sum(tg .* vtens);
