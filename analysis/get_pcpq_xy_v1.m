% change from original: output anc_flag

function [pcpq, cvec, cdot, anc_flag] = get_pcpq_xy(rbead,rmyo,rmyp,ifor,fmmat,bancf,bancm,r_ring,dbead_sq,vs_abs,dbead_first,vpol,d_for,d_myo)
...
% %     end
    ...
    anc_flag = true(1,length(cdot));
    anc_flag((length(cd1)+1):length(cd1)+length(cd2)) = false;
end