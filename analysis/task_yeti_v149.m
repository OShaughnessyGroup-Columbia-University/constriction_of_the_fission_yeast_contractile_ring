...
% change from v111: use parameters_v76, name bind2d7
% change from v118: use parameters_v84, name bind2d8
% change from v118: use parameters_v91, name bind_adia
% change from v144: use parameters_v91, name bind_adia2, TAKES ITASK=10 ALWAYS
% Adiabatically lowers binding force.
...
for t = loadj:tprint:loadj + totaltime-1
    ...
    if(abs(myo2p_unbinding_force - f_un_myo_fin) > 1e-6)
        myo2p_unbinding_force = myo2p_unbinding_force - df_myo;
        kcap = myo2p_unbinding_force / (rcapmyo_short-r_flat_myo);
        disp(['myo2 unbinding force: ' num2str(myo2p_unbinding_force)])
    end
    if(abs(myp2p_unbinding_force - f_un_myp_fin) > 1e-6)
        myp2p_unbinding_force = myp2p_unbinding_force - df_myp;
        kcap_p = myp2p_unbinding_force / (rcapmyp-r_flat_myp);
        disp(['myp2 unbinding force: ' num2str(myp2p_unbinding_force)])
    end
    disp(['Time: ' num2str(t)]);
    dt = .2; % seconds
    nstep = tprint/dt;
    equil_time_min = 0;
    real_t_min = t / 60 - equil_time_min; % real time, in minutes
    ...
    tsave = (t+tprint);
    clear t
    save(strcat(prefix,num2str(itask),'_',num2str(tsave),suffix), ...
        'real_t_min','rbead','rmyo','rmyp','ifor','ipt','bancf','bancm',...
        'xmat','fmmat','dbead_first','r','r_ring','fc','fanc',...
        'binding_rate_myo2','binding_rate_myp2','binding_rate_for','vpol',...
        'rsev','vbead','vmyo','force_vec','myo2p_unbinding_force','kcap',...
        'myp2p_unbinding_force','kcap_p')
    if r_ring < 0
        break
    end
end
