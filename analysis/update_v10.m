...
function [rbead,rmyo,rmyp,xmat,fmmat,ipt,ifor,bancf,bancm,dbead_first,bpmat_trial] = update_v10(rbead,rmyo,rmyp,bpmat,xmat,rhoain,rxbind,rex_act,fmmat,bancf,bancm,ifor,ipt,kofffor,...
...
%                 r_new_for = rmyo(:,myo_ind);    
                ...
                for inuc = 1:1000
                    ...
                    temp = rbead - (r_new_for + 0.1 * dbead * vrand);
                    temp = sum(temp.*temp);
                    disp(sqrt(min(temp)))
                    % if vrand is not within 2*rex_act of any bead, end
                    if all(temp > (2*rex_act)^2)
                        break
                    end
                    if inuc == 1000
