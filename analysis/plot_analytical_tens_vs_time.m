...
real_t_min_array = 0:19;
r_ring_array = 1.85 - 0.07*real_t_min_array;
l_ring_array = 2*pi*r_ring_array;
...
for icomp = 1:numel(real_t_min_array)
    ...
    [binding_rate_myo2, binding_rate_myp2, binding_rate_cdc12, vpol, rsev, l_mean_goal] = ...
        modify_binding_rates_v3(real_t_min, binding_rate_myo2_init, binding_rate_myp2_init, binding_rate_for_init, kofffor, r, r_ring, rhof, koffmyo);
...
tens_analytical = 16 * (n_myo_exp*1. + n_myp_exp*1.)./r_ring_array/2/pi .* l_exp/2;
% tens_analytical = 16 * (1-vpol_exp/.24) .* (n_myo_exp*fhead + n_myp_exp*fheadmyp)./r_ring_array/2/pi .* l_exp/2;
% tens_analytical = 16*1.3 * (1-vpol_exp/.24) .* (n_myo_exp*fhead + n_myp_exp*fheadmyp)./r_ring_array/2/pi .* l_exp/2;
% phi = acosd(r_ring_array / 1.85);
% plot(phi,tens_analytical,'r-')
% plot(l_ring_array, tens_analytical, 'r-')
% axis([0,80,0,1500])
