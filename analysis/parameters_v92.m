...
% change from v71: 2d scan myo2 and myp2 unbinding forces
% v72 to v75 skipped
% change from v76: use new best values of fhead and fheadmyp
% v77 to 83 skipped
...
% initial values
myo2p_unbinding_force = 10.;
myp2p_unbinding_force = 10.; 
jtask = mod(itask, length(X2(:)));
jtask = jtask + length(X2(:))*(jtask == 0);
f_un_myo_fin = X1(jtask);
f_un_myp_fin = X2(jtask);
df_myo = (myo2p_unbinding_force - f_un_myo_fin)/12;
df_myp = (myp2p_unbinding_force - f_un_myp_fin)/12;
%% Constants
% Misc.
