% clear
% load('noagm_5_00sec.mat')
% load('noagm_30_60sec.mat')
%% get forces
dt = 0.2;
...
nsat_p = nsat;
% calculates pulling force, capture force, excluded volume force (from
% other nodes and myp2) for each node. Returned as a 3xNnode matrix, in
% cylindrical coordinates
[force,force_vec, gbeadfv, gppf, bmmatpt, bpmatpt, ...
    fpull, fcap, fxvo, fxvp, fxv0_mean] = log_force(rbead,rmyo,rmyp,rdiff,ifor,ipt,...
    bancm,bmmat,bpmat,nhead,fhead,fheadmyp,fone,nsat,rcapmyo_u, ...
    rcapmyo_long, rcapmyo_short,rcapmyp,kcap,kcap_p,r_flat_myo,r_flat_myp,...
    dbead,vmyo0,kod2,r_ring,kwall,kexc,kexc_po,rexc,rexc_n,rexc_po,...
    kex_act,rex_act,xmat,rx0,kx,d_cdc15,d_myo,geometry);

%% get constraint forces
[pcpq, cvec, cdot] = get_pcpq_xy(rbead,rmyo,rmyp,ifor,fmmat,bancf,bancm,...
    r_ring,dbead_sq,vs_abs,dbead_first,vpol,d_for,d_myo);

%% get drag matrix
amat = get_a_v1(rbead, rmyo, rmyp, bmmat, bpmat, gppf, rdiff, dt, kcap, ...
    kcap_p, ifor, ipt, bmmatpt, bpmatpt);
m = get_m_v1 (dt,kcap,kcap_p,ifor,ipt,rbead,rmyo,rmyp,bancf,bancm,bmmat,...
    bpmat,rdiff,gbeadfv,gppf,gb,gf,gm,gmsol,bmmatpt,bpmatpt);
m = m - amat;
tau_o_dt = 2;
tau = tau_o_dt * dt;
nbead = size(rbead,2);
...
%% update velocities
[vbead, vmyo, vmyp, fc, fanc] = velocity(pcpq,m,force,nbead,rmyo,cvec,...
    cdot,tau,anc_flag);

%% calculate anchor forces
fmanc = zeros(3, length(rmyo));
ffor = zeros(3, length(rmyo));
for inode = 1:length(rmyo)
    for idx_for = find(fmmat(:, inode))'
        i0 = 3*(ifor(idx_for) - 1) + 1;
        ffor(:, inode) = ffor(:, inode) + fanc(i0:i0+2);
    end
    
    i0 = 3*(length(rbead) + inode - 1) + 1;
    fmanc(:, inode) = fmanc(:, inode) + fanc(i0:i0+2);
end
phimyo = cart2pol(rmyo(1, :), rmyo(2, :));
rhat_myo = [cos(phimyo); sin(phimyo); zeros(1, length(rmyo))];
phihat_myo = [-sin(phimyo); cos(phimyo); zeros(1, length(rmyo))];
fmanc = [diag(rhat_myo'*fmanc), diag(phihat_myo'*fmanc), fmanc(3, :)'];
ffor = [diag(rhat_myo'*ffor), diag(phihat_myo'*ffor), ffor(3, :)'];


%% get forces
function [force,force_vec, gbeadfv, gppf, bmmatpt, bpmatpt, fpull, fcap, fxvo, fxvp, fxv0_mean] = log_force(rbead,rmyo,rmyp,rdiff,ifor,ipt,bancm,bmmat,bpmat,nhead,fhead,fheadmyp,fone,nsat,rcapmyo_u, rcapmyo_long, rcapmyo_short,rcapmyp,kcap,kcap_p,r_flat_myo,r_flat_myp,dbead,vmyo0,kod2,r_ring,kwall,kexc,kexc_po,rexc,rexc_n,rexc_po,kex_act,rex_act,xmat,rx0,kx,d_cdc15,d_myo,geometry)
    ...
    %% myo2 pulling stall force
        ...
        phihat_myo = [-sin(phimyo); cos(phimyo); zeros(1, length(rmyo))];
        rhat_myo = [cos(phimyo); sin(phimyo); zeros(1, length(rmyo))];
        fpull = [diag(rhat_myo'*fmyo) diag(phihat_myo'*fmyo) fmyo(3, :)'];
    ...
    %% myo2 capture force  
        ...
        for i = 1:numel(ib)
            ...
            fcphi = f_m2b'*phihat_myo(:, this_im);
            if(fcphi ~=0)
                fcmag = [fcmag fcphi];
            end
        end
        figure
        histogram(fcmag, -15:2:15)
        title('magnitude of capture force')
        dfmyo = fmyo - fmyo0;
        fcap = [diag(rhat_myo'*dfmyo), diag(phihat_myo'*dfmyo), dfmyo(3, :)'];
...
%     end
    ...
    %% myosin excluded volume
        ...
        fxv0_mean = mean(fxv0);
        figure
        histogram(fxv0, 0:2:20)
        title('magnitude of excluded volume force')
        dfmyo = fmyo - fmyo0;
        fxvo = [diag(rhat_myo'*dfmyo), ...
                diag(phihat_myo'*dfmyo), ...
                dfmyo(3, :)'];
