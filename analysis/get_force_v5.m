...
% change from v4: gppf is an input value instead of calculated from sfpf.
% This is mainly for Myo2-E1.

function [force, gbeadfv, gppf, bmmatpt, bpmatpt] = get_force_v4(rbead,rmyo,rmyp,rdiff,ifor,ipt,bancm,bmmat,bpmat,nhead,fhead,fheadmyp,fone,nsat,rcapmyo_u, rcapmyo_long, rcapmyo_short,rcapmyp,kcap,kcap_p,r_flat_myo,r_flat_myp,dbead,vmyo0,kod2,r_ring,kwall,kexc,kexc_po,rexc,rexc_n,kex_act,rex_act,xmat,rx0,kx,d_cdc15,d_myo,geometry,gppf_one)
    ...
    % myo2 pulling stall force
        ...
        temp = ones(size(sfpf_myo));
        temp(sfpf_myo < 1e-5) = 0;
        gppf = temp * gppf_one;
...
% % %         end
    ...
    %% myosin excluded volume
        ...
        for i = 1:size(rmyp,2)
            ...
            for j = 1:length(ind)
                % magnitude of force
                fmag = kexc_po * (rexc - delr(j));
                % unit vector of delr
                delrunit = [delx(j); dely(j); delz(j)];
                delrunit = delrunit / norm(delrunit);
                % vector force
                f = - fmag * delrunit;
                % incoporate into fmyo and fmyp
                rhat = rmyp(:,i) / norm(rmyp(:,i));
                fmyp(:,i) = fmyp(:,i) + sum(f.*rhat)*rhat;
                rhat = rmyo(:,ind(j)) / norm(rmyo(:,ind(j)));
                fmyo(:,ind(j)) = fmyo(:,ind(j)) - sum(f.*rhat)*rhat;
            end
        end
