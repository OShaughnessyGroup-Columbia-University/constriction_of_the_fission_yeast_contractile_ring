%% change from v1: myosin does not interact with all actin beads within its capture radius. Instead, it selects nhead randomly to interact with.
...
% %         end
% %     end
%% each myo2 node interacts with nhead actin filaments only
for i = 1:size(bmmat,2)  % loop over every myosin
    if sum(bmmat(:,i)) > nhead  % if this myo2 is interacting with more than nhead actin filaments, need to select 4
        possible_ind = find(bmmat(:,i))';
        bmmat(:,i) = false;
        bmmat(randsample(possible_ind,nhead),i) = true;
    end
end
