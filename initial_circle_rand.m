function [rbead, rmyo, ipt, ifor, bancf, bancm, xmat, fmmat] = initial_circle_rand (rhom, rhof, rhoain, r, la, wr, d_for, d_myo, dbead, rexc, rxbind, koffmyo, vpol, rsev)
    ...
    rsp = (r-d_myo) * ones(1,nsp); % sp means spine
    ...
    for i = 1:size(la,2)
        ...
        % perturb the detached region
            rsp(ind) = perturb(rsp(ind),head,0);
    end
...
%         rmyo(1:2,:) = rmyo(1:2,:) * (1 - d_myo /r); 
    ...
    nf = round(rhof * lr);
    fmmat = false(nf,nm);
    % check that there are enough myosins
    if nf > nm
        error('not enough myosins')
    end
    ...
    zf = nan(1,nf);
    for i = 1:nf
        % pick an available myosin
        available = sum(fmmat) < 1;
        temp = find(available);
        choice = temp(randi(length(temp)));
        xf(i) = rmyo(1,choice)+.01*rand;
        yf(i) = rmyo(2,choice);
        zf(i) = rmyo(3,choice);
        fmmat(i,choice) = true;
    end
    ...
    % probability for a filament to have length lf
        ...
        lfvec = lfvec * dbead; % 
        % probability 
        aa = (koffmyo + lfvec * rsev) .* exp(-lfvec .* (2*koffmyo + lfvec * rsev) / (2 * vpol));
        %aa = zeros(length(lfvec), 1);
        %aa(lfvec == 2.5) = 1;
    ...
    for i = 1:nf
        ...
        for j = 1:nb
            ...
            if xbead*xbead + ybead*ybead > (r-d_myo)^2
                ...
                if xbead*xbead + ybead*ybead > (r-d_myo)^2
                    ...
                    if xbead*xbead + ybead*ybead > r*r
                        %error('nothing works!')
                    end
                end
            end
    ...
    % make it symmetric
    xmat = or(xmat,xmat');
% % %     % one bead cannot be bound to more than one other bead
% % %         % # xlinkers per bead
% % %         nxpb = sum(xmat);
% % %         % beads that have more than one xlinkers
% % %         ind = find(nxpb > 1);
% % %         % go through each of such beads
% % %         for i = ind
% % %             % neighbors crosslinked
% % %             nbr = find(xmat(i,:));
% % %             % if the problem has already been fixed, i.e. there are 1 or no
% % %             % xlinker on this bead
% % %             if isempty(nbr) || length(nbr)==1
% % %                 continue
% % %             end
% % %             % delete crosslinks
% % %             xmat(i,nbr) = 0;
