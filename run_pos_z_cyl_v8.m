...
% change from v1: put in myp2 in the beginning
% v2 and v3 skipped
% change from v4: rhof set to 17.2 better value
% v5 to v7 skipped
%% Constants
...
% density of components
    ...
    rhop = 2000/nheadmyp/10; % 200/16;    % assuming tom's rings are 10 um long
    ...
    rhom_sim = rhom_sim*rhom;
    rhop_sim = rhop_sim*rhop;
...
for itrial = 1:1e5
    ...
    [rbead, rmyo, ipt, ifor, bancf, bancm, xmat, fmmat] = initial_circle_rand (rhom_sim, rhof, rhoain, r, la, wr, d_for, d_myo, dbead, rmin, rxbind, koff, vpol, rsev);
...
%         fmmat = logical.empty(0,0);
    ...
    %% test if the entire ring is bundled
        ...
        for i = 1:size(rmyo,2)
            xtemp = abs(rmyo(1,i)-rmyo(1,:)) < rmin;    % myosins that are within rmin of the current myosin in the x direction
            ytemp = abs(rmyo(2,i)-rmyo(2,:)) < rmin;
            ztemp = abs(rmyo(3,i)-rmyo(3,:)) < rmin;
            xytemp = and(xtemp, ytemp);
            xyztemp = and(xytemp,ztemp);
            xyztemp(i) = false;                         % now xyztemp are myosins within rmin of the current myosin in x,y and z directions
            if any(xyztemp)     % if there is any such candidate
                for j = find(xyztemp)
                    rmyo_diff = rmyo(:,j) - rmyo(:,i);
                    rmyo_diff_sq = rmyo_diff' * rmyo_diff;
                    if rmyo_diff_sq < rmin * rmin
                        confbad = true;
                        break
                    end
                end
	...
	%[nList,dd] = rangesearch(rmyp', rmyp', rmin/2);
        %for k = 1:size(rmyp, 2)
        %    if length(nList{k}) > 2
        %        confbad = true
        %    end
