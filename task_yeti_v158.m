...
% change from v110: use parameters_v71, name wtfc3
...
%% run simulation
totaltime = 45*60; % ring radius divided by septation rate is 26 minutes  
warmuptime = .5;
...
if loadj==0
    ...
    [binding_rate_myo2, binding_rate_myp2, binding_rate_for, vpol, rsev]=...
        modify_binding_rates_v4(real_t_min, binding_rate_myo2_init, ...
        binding_rate_myp2_init, binding_rate_for_init, kofffor, r, r_ring, ...
        rhof, koffmyo);
    ...
    disp(['binding_rate_myo2: ' num2str(binding_rate_myo2)])
    disp(['binding_rate_for: '  num2str(binding_rate_for)])
    rk1_v41
    save(strcat(prefix,num2str(itask),'_0',suffix), 'real_t_min','rbead',...
        'rmyo','rmyp','ifor','ipt','bancf','bancm','xmat','fmmat',...
        'dbead_first','r','r_ring','fc','fanc','binding_rate_myo2',...
        'binding_rate_myp2','binding_rate_for','vpol','rsev','vbead','vmyo')
end

%% Maturation 
disp('***** Beginning maturation *****')
tmat = 20*60; % in seconds
tprint = 10; % seconds
for t = loadj:tprint:loadj + tmat - tprint
    load(strcat(prefix,num2str(itask),'_',num2str(t),suffix))
    dt = .2;
    nstep = tprint/dt;
    real_t_min = (t - tmat)/60.;
    ...
    tsave = t+tprint;
    clear t
    save(strcat(prefix,num2str(itask),'_',num2str(tsave),suffix), 'real_t_min','rbead','rmyo','rmyp','ifor','ipt','bancf','bancm','xmat','fmmat','dbead_first','r','r_ring','fc','fanc','binding_rate_myo2','binding_rate_myp2','binding_rate_for','vpol','rsev','vbead','vmyo','force_vec')
    if r_ring < 0
        break
    end
end
...
%% begin constriction
for t = loadj + tmat :tprint:loadj + tmat + totaltime - tprint
    load(strcat(prefix,num2str(itask),'_',num2str(t),suffix))
    dt = .2;
    nstep = tprint/dt;
    equil_time_min = 0;
    real_t_min = (t - tmat )/60; % real time, in minutes

    [binding_rate_myo2, binding_rate_myp2, binding_rate_for, vpol, rsev]=modify_binding_rates_v4(real_t_min, binding_rate_myo2_init, binding_rate_myp2_init, binding_rate_for_init, kofffor, r, r_ring, rhof, koffmyo);
    % septation rate (absolute value; defined as dr/dt; .07 micron / minute in Pelham and Chang)
    vs_abs = .07/60;
    rk1_v41
    tsave = t + tprint;
