...
parameters_v71
gmlist = [0.2, 0.5, 0.8, 1., 1.5];
xtask = mod(itask, 5);
gm = gm*gmlist(xtask+5*(xtask==0));
...
if loadj==0
    ...
    rk1_v32
    save(strcat(prefix,num2str(itask),'_0',suffix), 'real_t_min','rbead','rmyo','rmyp','ifor','ipt','bancf','bancm','xmat','fmmat','dbead_first','r','r_ring','fc','fanc','binding_rate_myo2','binding_rate_myp2','binding_rate_for','vpol','rsev','vbead','vmyo','gm')
end
tprint = 30;
totaltime = 30*60; % ring radius divided by septation rate is 26 minutes
for t = 0:tprint:totaltime-1
    load(strcat(prefix,num2str(itask),'_',num2str(t),suffix))
    dt = .2;
    nstep = tprint/dt;
    real_t_min = t/60; % real time, in minutes
    equil_time_min = 0;
    %nstep = 5*60;
    %equil_time_min = 0;
    ...
    tsave = t+tprint;
    clear t
    save(strcat(prefix,num2str(itask),'_',num2str(tsave),suffix), 'real_t_min','rbead','rmyo','rmyp','ifor','ipt','bancf','bancm','xmat','fmmat','dbead_first','r','r_ring','fc','fanc','binding_rate_myo2','binding_rate_myp2','binding_rate_for','vpol','rsev','vbead','vmyo','force_vec','gm')
    if r_ring < 0
        break
    end
end
