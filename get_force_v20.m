% use nheadmyp and nsatmyp. increase repulsion b/w myp clusters and nodes
function [force,force_vec,gbeadfv,gppf,bmmatpt,bpmatpt]=get_force_v10(rbead,rmyo,rmyp,rdiff,ifor,ipt,bancm,bmmat,bpmat,nhead,nheadmyp,fhead,fheadmyp,fone,nsat,nsatmyp,rcapmyo_u,rcapmyo_long,rcapmyo_short,rcapmyp,kcap,kcap_p,r_flat_myo,r_flat_myp,dbead,vmyo0,kod2,r_ring,kwall,kexc_oo,kexc_pp,kexc_po,rexc,rexc_n,rexc_po,kex_act,rex_act,xmat,rx0,kx,d_cdc15,d_myo,geometry)
    ...
    % myp2 pulling stall force
        % stall force per filament of each myosin cluster, full vector
        sfpf_myp = stall_force_pf (bpmat, fheadmyp*nheadmyp/nsatmyp, nsatmyp,nheadmyp,fheadmyp);
        % total stall force amplitude on each bead, full vector
    ...
    % myo2 capture force  
        ...
        for i = 1:numel(ib)
            ...
            %f_m2b = kcap * rbm_perp;
            dbm_perp = sqrt(sum(rbm_perp.*rbm_perp));
            rbm_perp = rbm_perp./repmat(dbm_perp, [3 1]);
            f_m2b = kcap * rbm_perp * max(0, dbm_perp - r_flat_myo);
    ...
    % myp2 capture force  
        ...
        for i = 1:numel(ib)
            ...
            dbm_perp = sqrt(sum(rbm_perp.*rbm_perp));
            rbm_perp = rbm_perp./repmat(dbm_perp, [3 1]);
            f_m2b = kcap_p * rbm_perp * max(0, dbm_perp - r_flat_myp);
            fbead(:, this_ib) = fbead(:, this_ib) + f_m2b;
    ...
    % actin filament bending force
        ...
        for i = fi
            ...
            if false   % the first segment after a formin has a different length
...
%     end
    ...
    %% myosin excluded volume
        ...
        for i = 1:size(rmyp, 2)
            ...
            % go through every myp2 cluster within rexc
            for j = 1:length(ind)
                % magnitude of force
                fmag = kexc_pp * (rexc - delr(j)) *(nheadmyp/16)^2;
                % unit vector of delr
                delrunit = [delx(j); dely(j); delz(j)];
                delrunit = delrunit / norm(delrunit);
                % vector force
                f = - fmag * delrunit;
                % incoporate into fmyp
                fmyp(:, i) = fmyp(:, i) + f;
            end
        end
        %% node-myp2 repulsion
    % go through every myo2 cluster
        for i = 1:size(rmyo, 2)
            ...
            rhat = [rmyo(1:2,i); 0] / norm(rmyo(1:2,i));
            for j = 1:length(ind)
                % magnitude of force
        ...
        temp = act_excl_force_v3gauss(rbead, kex_act, rex_act, ...
                                 sqrt(dbead*dbead/4+4*rex_act*rex_act), ifor);
        fbead = fbead + temp;
