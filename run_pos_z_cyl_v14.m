...
% v5 to v7 skipped
% v14 starts with ONE node and ONE formin
%% Constants
...
% density of components
    nsp = 2.5;
    rhom = 7.5 * nsp ; % initial density for setting binding rates
    rhom_real = 1/lr ; % actual initial density
    %rhop = 0;%200/16;
    rhof = 17.2*360/400; % initial density for setting binding rates
    rhof_real = 1/lr ; % actual initial density
    rhoain = 0;%25;
    
for itrial = 1:1e5
    
    %% Initial positions of all molecules
    [rbead, rmyo, ipt, ifor, bancf, bancm, xmat, fmmat] = ...
        initial_circle_rand_v2(rhom_real, rhof_real, rhoain, r, la, wr, d_for, ...
        d_myo, dbead, rexc, rxbind);
    rmyp = double.empty(3,0);
    %rmyp = nan(3,round(rhop*2*pi*r));
    %for i = 1:size(rmyp,2)
    %    temp = rand * 2 * pi;
