...
% change from v16: track entry times
% change from v22: use smart node selection scheme for formin addition
% change from v23: nucleate filaments along phihat
    ...
    bpmat_trial = bpmat;
        ...
        for i = 1:nmyo2_in
            for ibind = 1:100
				...
				t_rmyo = [t_rmyo,real_t_min + rand/10000];
                bancm = [bancm, true];
        ...
        disp(['update removing ' num2str(numel(temp)) ' overstretched segs'])
        if ~isempty(temp)
            temp = temp(1);
            temp = find(ifor<=temp);
            temp = temp(end);
...
%                 r_new_for = rmyo(:,myo_ind);    
                ...
                for inuc = 1:100
                    ...
                    x = rand;
                    if x < temp(myo_ind)/4 % each node has four binding sites
                        continue
                    end
				...
				t_rbead = [t_rbead, real_t_min + rand/10000, real_t_min + rand/10000];				
                ipt = [ipt, nbead + 2];
                ifor = [ifor, nbead + 1];
                nbead = nbead + 2;
...
%                             disp(itrial)
                ...
                rmyp = [rmyp,r_new];
				t_rmyp = [t_rmyp,real_t_min + rand/10000];
    ...
    if ~isempty(rbead)
        ...
        for ipoly = numel(poly_ind) : -1 : 1            % reverse order, convenient for insersion operations
			...
			t_rbead = [t_rbead(1:ifor(i)), real_t_min + rand/10000, t_rbead(ifor(i)+1:end)];
