function [rbead,rmyo,rmyp,xmat,fmmat,ipt,ifor,bancf,bancm,dbead_first,bpmat_trial] = update(rbead,rmyo,rmyp,bpmat,vbead,vmyo,xmat,fmmat,bancf,bancm,ifor,ipt,kofffor,...
    koffmyo,koffmyp,dt,d_for,d_myo,dbead,dbead_first,rsev,koffx,rcapmyp,l_break_sq,r,r_ring,wr,binding_rate_myo2,binding_rate_myp2,binding_rate_for,vpol,binding_rate_x,geometry);
...
% % %     % polymerization (periodic)        
% % %     if poly_time
% % %         new_end_rbead = 2*rbead(:,ipt) - rbead(:,ipt-1);
% % %         for i_insert = numel(ipt):-1:1
% % %             rbead = [rbead(:,1:ipt(i_insert)), new_end_rbead(:,i_insert), rbead(:,ipt(i_insert)+1:end)];            
% % %         end
% % %         bf = bancf(ifor);
% % %         ifor = ifor + (0:numel(ifor)-1);
% % %         ipt = ipt + (1:numel(ipt));
% % %         nbead = nbead + numel(ipt);
% % %         bancf = false(1,nbead);
% % %         bancf(ifor(bf)) = true;
% % %     end
...
%             elseif geometry == 'sph'
%                 for temp = 1:1e5
%                     z_in = zmin + rand * (zmax-zmin);
%                     weight = sqrt(r*r - z_in*z_in);
%                     if rand * weight_max < weight
%                         break
%                     end
%                 end
%                 [x_in, y_in] = pol2cart(theta,r_ring);
%                 r_in = [x_in; y_in; z_in];
%                 r_in = r_in * (1-d_myo / norm(r_in));
%                 rmyo = [rmyo, r_in];
%                 fmmat = [fmmat, false(size(fmmat,1),1)];
%             end
%             bancm = [bancm, true];
