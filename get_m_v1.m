...
function m = get_m_v1 (dt,ga_myo2,ga_myp2,ifor,ipt,rbead,rmyo,rmyp,bancf,bancm,bmmat,bpmat,rdiff,gbeadfv,gppf,gb,gf,gm,gmsol,bmmatpt,bpmatpt)
...
B =
   ...
   res =  A .* B(ones(1, 3),:);
    ...
    for i = 1:size(rmyo,2)
		...
		[theta,rho_waste,z_waste] = cart2pol(rmyo(1,i),rmyo(2,i),rmyo(3,i));
		
	    % adjust drag coefficients in the division plane to ensure that
	    % drag coefficient along the radial direction is just gmsol
	    % and not gm + gmsol.
	    g1 = gmsol * (cos(theta))^2 + gm * (sin(theta))^2;
	    g2 = (-gm + gmsol) * (cos(theta) * sin(theta));
	    g3 = gm * (cos(theta))^2 + gmsol * (sin(theta))^2;
