...
% change from v135: rk1_v40, name ptanc
% v125 to 134 skipped
...
%% run simulation
totaltime = 60*10; % ring radius divided by septation rate is 26 minutes  
warmuptime = .5;
...
%     dt = .005; 
    ...
    rk1_v40
    save(strcat(prefix,num2str(itask),'_0',suffix), 'real_t_min','rbead','rmyo','rmyp','bmmat','bpmat','ifor','ipt','bancf','bancm','xmat','fmmat','dbead_first','r','r_ring','fc','fanc','binding_rate_myo2','binding_rate_myp2','binding_rate_for','vpol','rsev','vbead','vmyo');
end

vmyo0 = 1e6; % kill v dependence in fv reln
tprint = 2;
...
%    nstep = 5;
    ...
    tsave = t+tprint;
    clear t
    save(strcat(prefix,num2str(itask),'_',num2str(tsave),suffix), 'real_t_min','rbead','rmyo','rmyp','bmmat','bpmat','ifor','ipt','bancf','bancm','xmat','fmmat','dbead_first','r','r_ring','fc','fanc','binding_rate_myo2','binding_rate_myp2','binding_rate_for','vpol','rsev','vbead','vmyo');
    if r_ring < 0
        break
    end
end
