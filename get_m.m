function w = get_m (ga,ifor,ipt,rbead,rmyo,rmyp,bancf,bancm,bmmat,bpmat,rdiff,gbeadfv,gppf,gb,gf,gm,gmsol,bmmatpt,bpmatpt)
...
% block-diagonal form.
    ...
    % for beads
    if ~isempty(rbead)
        totdrag_vec = ga * (sum(bmmatpt,2)+sum(bpmatpt,2));
    else
        totdrag_vec = [];
    end
	...
	% for myo2
	km = gmsol * ones(1,nm) +  ga * sum(bmmatpt,1); % solution drag
	...
	kp = gmsol * ones(1,np) + ga * sum(bpmatpt,1);
...
B =
   ...
   res =  A .* B(ones(1, 3),:);
	...
	w = sparse(rows(:)',cols(:)',A(:),3*(nb+nm+np),3*(nb+nm+np));
end