...
function F = act_excl_force_v3(rbead,kex,rex,rcut,ifor)
    ...
    for k = 1:nbead
        ...
        %good = (nList{k} > k+1 & ...
        %    (ismember(nList{k}, nList{k-1}) & ismember(nList{k}-1, nList{k})) &...
        %    ~ismember(nList{k}, ifor) & ~ismember(nList{k}, ifor+1));
        %nListVec = nList{k}(good);

        nlbrb = union(nList{k}, nList{k}+1);
        nlpt = union(nList{k-1}, nList{k-1}+1);
        nl = union(nlbrb, nlpt);
        good = (nl > k+1 & ~ismember(nl, ifor) & ~ismember(nl, ifor+1));
        nListVec = nl(good);
        %nListVec = intersect(nl, nl+1); % only look at barbed ends
        ...
        for l = 1:length(nListVec)
          ...
          % F1 is a 1x3 vector
          [F1, s, t] = getSegForceGauss(k, nListVec(l), rbead, kex, rex);
