...
vsvec = [1, 2, 3, 5, 10, 20];
xtask = mod(itask, 6)
vfactor = vsvec(xtask + 6*(xtask==0));
tprint = 5;
totaltime = 30*60/vfactor; % ring radius divided by septation rate is 26 minutes
for t = 0:tprint:totaltime-1
    load(strcat(prefix,num2str(itask),'_',num2str(t),suffix))
    dt = .2;
    nstep = tprint/dt;
    equil_time_min = 0;
    real_t_min = 10; % constant densities
    %real_t_min = t/60*vfactor; % use densities for current ring radius
    %real_t_min = t/60; % real time, in minutes
    %real_t_min = t * dt * nstep / 60 - equil_time_min; % real time, in minutes

    if real_t_min >=0
        [binding_rate_myo2, binding_rate_myp2, binding_rate_for, vpol, rsev]=modify_binding_rates_v2(real_t_min, binding_rate_myo2_init, binding_rate_myp2_init, binding_rate_for_init, kofffor, r, r_ring, rhof, koffmyo);
        % septation rate (absolute value; defined as dr/dt; 
        % .07 micron / minute in Pelham and Chang)
        vs_abs = .07/60*vfactor ;
    end
    rk1_v32
    tsave = t+tprint;
    clear t
    save(strcat(prefix,num2str(itask),'_',num2str(tsave),suffix), 'real_t_min','rbead','rmyo','rmyp','ifor','ipt','bancf','bancm','xmat','fmmat','dbead_first','r','r_ring','fc','fanc','binding_rate_myo2','binding_rate_myp2','binding_rate_for','vpol','rsev','vbead','vmyo','force_vec', 'vs_abs')
    if r_ring < 0
        break
    end
end
