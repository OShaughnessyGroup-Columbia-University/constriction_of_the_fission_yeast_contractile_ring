...
itask = 10;
FUVEC = [2., 3., 3.5];
xtask = mod(itask, length(FUVEC));
fup = FUVEC(xtask + (xtask==0)*length(FUVEC) );
parameters_v91
...
for t = loadj:tprint:loadj + totaltime-1
    ...
    if(myp2p_unbinding_force > fup + 1e-6)
        myp2p_unbinding_force = myp2p_unbinding_force - 0.05;
        kcap_p = myp2p_unbinding_force / (rcapmyp-r_flat_myp);
        disp(['unbinding force: ' num2str(myp2p_unbinding_force)])
    end
    disp(['Time: ' num2str(t)]);
    dt = .2; % seconds
    nstep = tprint/dt;
    ...
    save(strcat(prefix,num2str(itask),'_',num2str(tsave),suffix), ...
        ...
        'rsev','vbead','vmyo','force_vec','myp2p_unbinding_force','kcap_p')
    if r_ring < 0
        break
    end
end
