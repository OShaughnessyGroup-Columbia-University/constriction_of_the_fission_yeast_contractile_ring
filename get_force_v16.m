...
% change from v10: use act_excl_force_v3, fixed rhat 3rd law problem
% change from v14: use different values of node-node and myp-myp spring consts
% change from v15: use constant force for node-node xvol
function [force,force_vec,gbeadfv,gppf,bmmatpt,bpmatpt]=get_force_v10(rbead,rmyo,rmyp,rdiff,ifor,ipt,bancm,bmmat,bpmat,nhead,fhead,fheadmyp,fone,nsat,rcapmyo_u,rcapmyo_long,rcapmyo_short,rcapmyp,kcap,kcap_p,r_flat_myo,r_flat_myp,dbead,vmyo0,kod2,r_ring,kwall,fexc_oo,kexc_oo,kexc_pp,kexc_po,rexc,rexc_n,rexc_po,kex_act,rex_act,xmat,rx0,kx,d_cdc15,d_myo,geometry)
...
%     end
    ...
    %% myosin excluded volume
        ...
        for i = 1:size(rmyo, 2)
            ...
            for j = 1:length(ind)
                % magnitude of force
                fmag = fexc_oo;%kexc_oo * (rexc_n - delr(j));
                % unit vector of delr
    ...
    % go through every myo2 cluster
        ...
        temp = act_excl_force_v3(rbead, kex_act, rex_act, ...
                                 sqrt(dbead*dbead/2+4*rex_act*rex_act), ifor);
        fbead = fbead + temp;
