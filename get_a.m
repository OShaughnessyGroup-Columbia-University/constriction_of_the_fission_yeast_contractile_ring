function amat = get_a(rbead, rmyo, rmyp, bmmat, bpmat, gppf, rdiff, ga, ifor, ipt, bmmatpt, bpmatpt)
...
% % %     arow(ind:ind+n-1) = grow' + 3 * (ib-1);
% % %     acol(ind:ind+n-1) = gcol' + 3 * (im-1);
% % %     aval(ind:ind+n-1) = gval';
% % %     arow(ind+n:ind+2*n-1) = grow' + 3 * (im-1);
% % %     acol(ind+n:ind+2*n-1) = gcol' + 3 * (ib-1);
% % %     aval(ind+n:ind+2*n-1) = gval';
% % %     % update ind
% % %     ind = ind + 2*n;
% % % end
...
rows = bsxfun(@(x,y) y*x,ones(1,3),(1:3*(nb+nm))')';
t1 = bsxfun(@(x,y) y*x,ones(1,3),(1:3:3*(nb+nm))')';
t2 = t1(:)';
cols = [t2;t2+1;t2+2];
