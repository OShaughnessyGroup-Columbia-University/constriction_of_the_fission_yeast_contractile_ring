...
% change from v6: incoming Myo2p avoids collision
% v7 skipped

function [rbead,rmyo,rmyp,xmat,fmmat,ipt,ifor,bancf,bancm,dbead_first,bpmat_trial] = update_v8(rbead,rmyo,rmyp,bpmat,xmat,rhoain,rxbind,fmmat,bancf,bancm,ifor,ipt,kofffor,...
    koffmyo,koffmyp,atp_cycle,d_for,d_myo,dbead,dbead_first,rsev,koffx,rcapmyp,l_break_sq,r_ring,wr,binding_rate_myo2,binding_rate_myp2,binding_rate_for,vpol,rexc_n);
...
% % %     nbefore = sum(xmat(:)) * .5;
    ...
    bpmat_trial = bpmat;
        ...
        nmyoturn = koffmyo * size(rmyo,2) * atp_cycle;
        nmypturn = koffmyp * size(rmyp,2) * atp_cycle;
        % make it an integer
        ...
        nmyo2_in = binding_rate_myo2 * 2 * pi * r_ring * atp_cycle;                % number of myosin clusters to come in
        ...
        for i = 1:nmyo2_in
            for j = 1:1e4;
                ...
                ydist = rmyo(2,:) - y_in;
                zdist = rmyo(3,:) - z_in;
                min_dist_sq = xdist.*xdist + ydist.*ydist + zdist.*zdist;
                if min_dist_sq > (rexc_n/2)^2
                    disp(j)
                    break
                end
                if j == 1000
                    error('cannot find location to insert Myo2p')
                end
            end
        ...
        nforturn = kofffor * atp_cycle * length(ifor);
        % make it an integer
        ...
        nfor_in = binding_rate_for * 2 * pi * r_ring * atp_cycle;                     % number of formin dimers to nucleate
...
%                 r_new_for = rmyo(:,myo_ind);    
    ...
    %% cofilin severing
        ...
        nmyp2_in = binding_rate_myp2 * 2 * pi * r_ring * atp_cycle;                           % number of myosin clusters to come in
    ...
    if ~isempty(rbead)
        ...
        nxturn = koffx * atp_cycle * .5 * full(sum(sum(xmat)));
        % make it an integer
        ...
        xmat = sparse(xrow,xcol,xval,nbead,nbead);
        dbead_first = dbead_first + vpol * atp_cycle;
        if isempty(rbead)
