...
% change from v16: put fo

function [rbead,rmyo,rmyp,xmat,fmmat,ipt,ifor,bancf,bancm,dbead_first,bpmat_trial] = update_v10(rbead,rmyo,rmyp,bpmat,xmat,rhoain,rxbind,rex_act,rexc_n,fmmat,bancf,bancm,ifor,ipt,kofffor,...
...
%                 r_new_for = rmyo(:,myo_ind);    
                ...
                for inuc = 1:100
                    ...
                    nfor_node = temp(myo_ind);
                    if nfor_node>3              % cannot bind to myosin already occupied by 4 formin dimers
                        continue
                    end
                    ...
                    if all(temp > (0.7*rex_act)^2)
                        break
                    end
                    if inuc == 100
                        disp(['nmyo:' num2str(length(rmyo))])
                        disp(['nfor_in:' num2str(nfor_in)])
                        disp(['nfor:' num2str(length(ifor))])
                        disp(['nfor_node:' num2str(nfor_node)])
